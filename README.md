...
-   [Video Reconstruction with CLIP, HDBSCAN and TSP Optimization](#video-reconstruction-with-CLIP-HDBSCAN-and-TSP-Optimization)
...
This project implements **Stochastic Variational Inference (SVI) for Latent Dirichlet Allocation (LDA)**, following Hoffman et al. (2013). It compares **SVI with traditional CAVI** on synthetic datasets of increasing size to evaluate convergence speed and scalability. Both methods are benchmarked using ELBO tracking and runtime analysis, showing that while CAVI achieves marginally better ELBO, **SVI dramatically improves computational efficiency** and handles large-scale data effectively.
...
The `vision` folder investigates **core and advanced concepts in computer vision**, from classical image processing techniques to modern neural architectures and multimodal frameworks. Projects progress from frequency-based analysis and edge detection to 3D reconstruction, deep convolutional models, and semantic video understanding. Each implementation emphasizes both **mathematical intuition and empirical experimentation**, combining tools from signal processing, geometry, and deep learning to explore how visual information can be extracted, represented, transformed, and interpreted. Together, these works form a coherent study of how computers can **perceive, analyze, and reconstruct visual data** at increasing levels of abstraction and complexity.

----------

### Filtering Operations

This project explores the use of the **Discrete Fourier Transform (DFT)** for image processing, with a focus on **frequency-based filtering**. Through hands-on experiments, it investigates how different frequencies contribute to image content and how filters like Gaussian and median impact noise reduction and visual clarity. The study highlights the visual artifacts caused by **aliasing** and the benefits of **smoothing prior to subsampling**. Comparisons between filtering methods show tradeoffs between noise removal and preservation of image details.
...
This project develops a **full unsupervised video reconstruction pipeline that restores the temporal coherence** of corrupted or shuffled videos. Built on **OpenAI’s CLIP** for high-level visual embeddings, **HDBSCAN** for clustering, and **graph optimization heuristics** for sequence ordering, the method combines semantic understanding and combinatorial reasoning to recover smooth playback from unordered frames. By combining high-level visual understanding with combinatorial optimization, the method **robustly recovers scene continuity** even under severe frame disorder. The implementation is efficient, modular, and fully based on **NumPy, PyTorch, and OpenCV**.

[Go to Video Reconstruction with CLIP, HDBSCAN and TSP Optimization](./vision/video_reconstructor/README.md)
